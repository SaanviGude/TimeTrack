"""
Migration script to add soft delete columns to workspace_members table
...
def run_migration():
    """Add soft delete columns to workspace_members table"""
    try:
        ...
        with engine.begin() as conn:  # Use begin() instead of connect()
            ...
            if 'is_deleted' in existing_columns and 'deleted_at' in existing_columns:
                print("Soft delete columns already exist in workspace_members table!")
                return
            ...
            conn.execute(text("""
                COMMENT ON COLUMN workspace_members.is_deleted IS 'Soft delete flag';
            """))
            conn.execute(text("""
                COMMENT ON COLUMN workspace_members.deleted_at IS 'Timestamp when the member was deleted';
            """))

            # Transaction will auto-commit when exiting the context
            print("✅ Migration completed successfully!")
            print("Soft delete columns added to workspace_members table.")
...
def rollback_migration():
    """Remove soft delete columns from workspace_members table"""
    try:
        with engine.begin() as conn:  # Use begin() instead of connect()
...
def check_table_exists():
    ...
    try:
        with engine.connect() as conn:
            ...
            return 'workspace_members' in tables
...
def check_table_structure():
    ...
    try:
        with engine.connect() as conn:
            ...
            print("\n📋 Current workspace_members table structure:")
            print("-" * 80)
            print(
...
if __name__ == "__main__":
    print("=== Workspace Members Soft Delete Migration ===")

    # Check if table exists first
    if not check_table_exists():
        print("❌ workspace_members table does not exist!")
