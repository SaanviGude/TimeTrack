...
class ProjectMemberRole(BaseSchema):
    role: ProjectRole = Field(
        ProjectRole.MEMBER, description="Role of the member in the project team")


class ProjectMemberCreate(ProjectMemberRole):
    user_id: uuid.UUID = Field(...,
                               description="ID of the user to add to the project team")


class ProjectMemberUpdate(ProjectMemberRole):
    role: Optional[ProjectRole] = Field(
        None, description="New role for the project member")
...
class ProjectResponse(ProjectBase, BaseDBSchema):
    ...
    members: List[ProjectMemberResponse] = Field(
        [], description="List of members in this project team")
